...
class Main(tk.Tk):
    ...
    def __init__(self, *args, **kwargs): #Initialising attributes of class
        ...
        self.windows["continue_to_gymstore"] = continue_to_gymstore( manager=self, parent=manager) #Creates continue_to_gymstore window
        ...
        self.windows["continue_to_gymstore"].grid(row=0, column=0, sticky="nsew")#Geometry manager organising widgets for continue_to_gymstore
    ...
    def login_verify(self):
        ...
        if self.verify_gmail in list_of_files: 
            ...
            if self.verify_password in verify:
                self.manager.show_window("continue_to_gymstore")
            else:   
...
class continue_to_gymstore(tk.Frame): #continue_to_gymstore class
    def __init__(self, parent, manager): #Initializes attributes
        ...
        home_page.banner(self) #Banner
        self.continue_button = tk.Button(self, text = "Continue to Cafe Menu", height = 5, width = 20, command = self.continue_to_store) #Continue button
        self.continue_button.place(x=110, y= 170)
        self.delete_button = tk.Button(self, text = "Delete", fg= 'red', command = self.delete) #Delete account button
        self.delete_button.place(x=15,y=360)
        self.back_to_home_button = tk.Button(self, text = "Home", command = lambda: manager.show_window("home_page"))#Back to startpage button
        self.back_to_home_button.place(x=120,y=360) 
    def delete(self): #Delete account method
        ...
        if delete_user_ask == 'yes':
            ...
            return
    def continue_to_store(self):
        app.destroy()
        os.system('python Final_Version_Gym_Store.py')
